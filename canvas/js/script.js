...
function updateGame() {
    if(game.state == "playing" && enemies.length == 0) {
        game.state = "won";
        overlay.title = "РОЙ МЕРТВЕЦОВ";
        overlay.subtitle = "нажмите пробел, чтобы снова играть";
        overlay.counter = 0;
    }
...
function updatePlayer() {
 ...
 addEventListener("keydown", function(event, soundClick) {
    ...
    if(game.state == "start") {
        ...
        for(var i=0; i<10; i++) {
            enemies.push({
                    ...
                    phase: Math.floor(Math.random()*90), //make the enemies not be identical
                    shrink: 20,
            });
        }
        game.state = "playing";
